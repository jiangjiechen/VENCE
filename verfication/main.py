...
def entity_position_com(claim, entity_position, sp_token):
    ...
    return_position = []
    tokenizer_list = [t[1:] if t[0] ==
                      'Ġ' else t for t in tokenizer.tokenize(claim)]
    entity_dict = {}
    ...
    for p in sp_token:
        for ent in temp_entity_position:
            if ''.join([tokenizer_list[cc] for cc in p]) == ent:
                ...
                temp_entity_position.remove(
                    ''.join([tokenizer_list[cc] for cc in p]))
...
def entity_position_com2(claim, entity_position):
    ...
    for entity in entity_position:
        entity_list_raw = [t[1:] if t[0] ==
                           'Ġ' else t for t in tokenizer.tokenize(entity)]
...
def token_logit(claim, evidence, claim_entity):
    ...
    for name, param in model.named_parameters():
        if param.requires_grad and name == 'roberta.encoder.layer.23.output.LayerNorm.weight':
            ...
            token_pos = [
                item for item in token_pos if item not in sp_token_sum]
            for n in token_pos:
                token_sum = {
                    'token': tokenizer_list_raw[n], 'id': id_token, 'pos': n, 'prob': temp_prob[0][n].item()}
            ...
            for p in sp_token_pos:
                ...
                if temp_p in claim_entity:
                    entity_sum = {'entity': temp_p, 'id': id_entity,
                                  'pos': p, 'prob': temp_prob[0][p[0]:p[-1]].mean()}
                    ...
                    for ce in claim_entity:
                        if temp_p in ce:
                            token_flag = False
                    if token_flag:
                        p_token_sum = {'token': temp_p, 'id': id_token,
                                       'pos': p, 'prob': temp_prob[0][p[0]:p[-1]].mean()}
            ...
            for e in entity_pos:
                if type(e) == int:
                    entity_sum = {
                        'entity': tokenizer_list_raw[e], 'id': id_entity, 'pos': e, 'prob': temp_prob[0][e]}
                    if entity_sum['pos'] not in [mt['pos'] for mt in final_prob['entity']]:
                        final_prob['entity'].append(entity_sum)
                        id_entity += 1
                else:
                    ...
                    for _, eee in enumerate(e):
                        if _ != 0:
                            if eee not in he_nn:
                                zb_entity = zb_entity+' ' + \
                                    tokenizer_list_raw[eee]
                            else:
                                zb_entity = zb_entity+tokenizer_list_raw[eee]
                        else:
                            zb_entity = tokenizer_list_raw[eee]
                    entity_sum = {'entity': zb_entity, 'id': id_entity,
                                  'pos': e, 'prob': temp_prob[0][e[0]:e[-1]].mean()}
                    if entity_sum['pos'] not in [mt['pos'] for mt in final_prob['entity']]:
                        final_prob['entity'].append(entity_sum)
                        id_entity += 1
...
if __name__ == '__main__':
    ...
    print(entity_position_com2('Watertown, Massachusetts is in Massachusetts', [
