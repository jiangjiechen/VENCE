...
def generate(text,candi_token=''):
    ...
    outputs = model.generate(inputs['input_ids'], max_length=100, num_beams=5, early_stopping=True,output_scores=True,return_dict_in_generate=True)
    candi_token_id=tokenizer.convert_tokens_to_ids(tokenizer.tokenize(candi_token))
...
def entity_pro_generate(text,entity_list):
    ...
    for entity in entity_list:
        text_tensor=torch.tensor([tokenizer.convert_tokens_to_ids(tokenizer.tokenize(text))])
        entity_tensor=torch.tensor([tokenizer.convert_tokens_to_ids(tokenizer.tokenize(entity))])
