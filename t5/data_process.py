...
if __name__ == '__main__':
    ...
    with jsonlines.open('../data/whitebox_ir_train_genre_50_2.jsonl') as reader, jsonlines.open('data/entity_loc.jsonl', mode='w') as writer:
        text_100 = split_list([[obj['sentence1'], obj['sentence2']]
                              for obj in reader if obj['original']['veracity'] == "SUPPORTS"], 100)
        num = 0
        for text in text_100:
            ...
            for _, sentence in enumerate(entity_extract(text_text)):
                entity_loc = [get_word_index(text_text[_], sentence[index][1], sentence[index][2]) for index in
                              range(len(sentence))]
                writer.write([text_text[_], [item[0]
                             for item in sentence], entity_loc, text_evidence[_]])
            num += 100
            print(num)
    ...
    with jsonlines.open('data/t5_data.jsonl', mode='w') as writer, jsonlines.open('data/entity_loc.jsonl') as reader:
        for obj in tqdm(reader):
            try:
                masked_entity = mask_words(
                    obj[0], obj[2], entity_or_word='entity')
                masked_word = mask_words(obj[0], obj[2], entity_or_word='word')
                writer.write({'input': 'substituted entity : ' +
                             '[evidence] : '+obj[-1]+' [claim] : '+masked_entity[0], 'output': obj[1][masked_entity[1]]})
                writer.write({'input': 'substituted one word : ' +
                             '[evidence] : ' + obj[-1] + ' [claim] : ' + masked_word[0], 'output': obj[0].split()[masked_word[1]]})
            except:
                pass
